...
def main():
    ...
    for query in desired_queries:
        ...
        for i in range(end_page):
            ...
            soup = create_soup_from_url(url=search_page_url + str(page) + '/?s=' + quote(query))

            # Extracting number of link_results
            search = soup.find('div', attrs={'class': 'archive-grid'}).findAll('article')

            # Search for articles within given tag:
            for s in search:
                articles = soup.find('div', attrs={'class': 'archive-grid'}).findAll('article')

                # Extract the link of each article:
                for a in articles:
                    links = a.find('h3', attrs={'class': 'article-title'}).find('a').get('href')
                    # print(links)
    ...
    for i in range(len(link_results)):
        ...
        # Get article body
        try:
            article_body = soup.find('div', attrs={'class': 'entry-content'}).find('section').findAll('p', recursive=False)
        except:
        ...
        # Get article date
        try:
            article_date = soup.find('header').find('time')
